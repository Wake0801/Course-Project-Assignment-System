...
public class LopTinChiController {
    ...
    @Autowired
    private LBC_LTCRepository loaiBaoCaoLopTCRepository;
    ...
    public String listLTC(
            ...
            @RequestParam(value = "sortNgayMoLop", required = false, defaultValue = "desc") String sortNgayLap,
        ...
        Page<LopTinChi> ltcPage = lopTinChiService.findLopTinChisAdvanced(
            keyword, hocKi, maGV, trangThai, page, size, sortNgayLap
        );
        ...
        model.addAttribute("sortNgayMoLop", sortNgayLap);
        ...
        if (editLTC != null && editLTC.getMaLopTC() != null) {
            var heSoList = loaiBaoCaoLopTCRepository.findByMaLopTC(editLTC.getMaLopTC());
            for (var h : heSoList) {
                editHeSoDiemMap.put(String.valueOf(h.getLoaiBaoCao().getMaLoaiBaoCao()), h.getHeSoDiem());
            }
        }
    ...
    public String saveLTC(
        ...
        if (ltc.getMonHoc().getMaMon() == null || ltc.getMonHoc().getMaMon().trim().isEmpty()) {
            redirectAttributes.addFlashAttribute("error", "Vui lòng chọn mã môn học!");
            redirectAttributes.addFlashAttribute("editLTC", ltc);
            redirectAttributes.addFlashAttribute("showModal", true);
            return "redirect:/ltc";
        }
        if (ltc.getGiangVien().getMaGV() == null || ltc.getGiangVien().getMaGV().trim().isEmpty()) {
        ...
        try {
            ...
            if (ltc.getNgayMoLop() == null) {
                ltc.setNgayMoLop(java.time.LocalDate.now());
            }
            if (!isUpdate) {
            ...
            for (LoaiBaoCao lbc : listLoaiBaoCao) {
                ...
                if (allParams.containsKey(key)) {
                    try {
                        ...
                        LoaiBaoCaoLopTCId id = new LoaiBaoCaoLopTCId(lbc.getMaLoaiBaoCao(), ltc.getMaLopTC());
                        loaiBaoCaoLopTCRepository.deleteById(id);
                        // Lưu mới
                        var entity = new LoaiBaoCaoLopTC();
                        entity.setMaLopTC(ltc);
                        entity.setLoaiBaoCao(lbc); 
    ...
    public ResponseEntity<?> getLTCInfo(@PathVariable("maLTC") String maLTC) {
        return lopTinChiService.findById(maLTC)
                .map(ltc -> {
                    ...
                    map.put("maMon", ltc.getMonHoc().getMaMon());
                    map.put("maGV", ltc.getGiangVien().getMaGV());
                    ...
                    map.put("ngayMoLop", ltc.getNgayMoLop() != null ? ltc.getNgayMoLop().toString() : "");
    ...
    public Map<String, Object> addStudentToLTC(@PathVariable("maLTC") String maLTC, @RequestBody Map<String, String> payload) {
        ...
        SinhVienLTCId pk = new SinhVienLTCId(maSV, maLTC);
    ...
    public List<Map<String, Object>> getHeSoDiemLopTC(@PathVariable("maLTC") String maLTC) {
        List<LoaiBaoCaoLopTC> list = loaiBaoCaoLopTCRepository.findByMaLopTC(maLTC);
        List<LoaiBaoCao> loaiBaoCaoList = loaiBaoCaoRepository.findAll();
        
        Map<Integer, String> tenLoaiBaoCaoMap = loaiBaoCaoList.stream()
            .collect(Collectors.toMap(
                LoaiBaoCao::getMaLoaiBaoCao,
                LoaiBaoCao::getTenLoaiBaoCao
            ));

        return list.stream().map(item -> {
            Map<String, Object> map = new HashMap<>();
            map.put("maLoaiBaoCao", item.getId().getMaLoaiBaoCao());
            map.put("tenLoaiBaoCao", tenLoaiBaoCaoMap.get(item.getId().getMaLoaiBaoCao()));
